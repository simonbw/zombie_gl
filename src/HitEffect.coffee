...
class window.HitEffect
    ...
    constructor: (@x, @y, @z, @vx, @vy, type = "concrete") ->
        @lifespan = LIFESPAN
...
#        dx = @vx / (Math.pow(@vx, 2) + Math.pow(@vy, 2) + 1) * -1
#        dy = @vy / (Math.pow(@vx, 2) + Math.pow(@vy, 2) + 1) * -1
#        @light = game.lightManager.getPointLight(@x + dx, @y + dy, @z)
